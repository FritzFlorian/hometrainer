...
def secure_client_connection(client, ctx, config: Configuration, only_localhost=False):
    ...
    client_public, client_secret = config.zmq_client_secret()
    client.curve_secretkey = client_secret
    client.curve_publickey = client_public

    server_public, _ = config.zmq_server_public()
...
def secure_server_connection(server, ctx, config: Configuration, only_localhost=False):
    ...
    server_public, server_secret = config.zmq_server_secret()
...
def plot_external_eval_avg_score(work_dir, lower_bound, upper_bound, show_progress_lines,
    ...
    plt.plot(x_steps, avg_score, linestyle='--')
    plt.plot(x_steps, smoothed_wins)
    plt.ylim(min, max)
    plt.xlabel('Batch')
    plt.ylabel('Average Game Outcome')
